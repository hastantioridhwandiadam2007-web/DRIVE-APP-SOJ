...
while true; do
    ...
    if [ "$SERVER_PASS" == "k5drive" ]; then
        # Jika benar, keluar dari loop dan lanjut ke program utama
        break
    else
        # Jika salah, tampilkan pesan error merah dan minta input ulang
        echo -e "\e[31m [-] Authentication failed. Incorrect password.\e[0m"
        echo -e "\e[34m--------------------------------------------------\e[0m"
    fi
done
